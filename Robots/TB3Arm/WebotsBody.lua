...
function WebotsBody.update(Body)
    ...
    if ENABLE_POSE then
      ...
      dcm.sensorPtr.rpy[0], dcm.sensorPtr.rpy[1], dcm.sensorPtr.rpy[2] =rpy[1], rpy[2], gps_pose[3]
			...
			--Broadcast pose
			pose_pub_count=pose_pub_count+1
			if pose_pub_count%pub_factor==0 then
			...
			if Config.use_fast_pose then wcm.set_robot_pose(gps_pose) end
    end
    ...
    -- Grab a lidar scan
      ...
      NEXT_HEAD_LIDAR = t + lidar_timeStep / 1000
		...
		while webots.wb_receiver_get_queue_length(tags.receiver) > 0 do end
