...
local all_servo_ids={1,2,3,4,5,6,11,12,13,14}
local position_servo_ids={1,2,3,4,5,6}
...
DXL.control_mode(position_servo_ids,3*vector.ones(#position_servo_ids) )
DXL.control_mode(velocity_servo_ids,vector.ones(#velocity_servo_ids) )
...
--
-- DXL.pro_torque_enable(all_servo_ids,vector.ones(6))
...
for i=1,400 do
  ret=DXL.pro_read_position(position_servo_ids)

  count=count+1
  if count%100==0 then
    local t=unix.time()
    local fps = count/(t-t_last)
    count,t_last=0,t
    print(fps," fps")
