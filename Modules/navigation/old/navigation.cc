...
int lomap_size=201; //30 by 30 meter with 15cm resolution
...
int8_t lomap[201*201];
int8_t temp_lomap[201*201];
int8_t dirmap[201*201];
float lomapv[201*201]; //value map
		...
		float dist1=sqrt((xpos-6.6)*(xpos-6.6) + (ypos-6.0)*(ypos-6.0));
	 	...
	 	if (current_value>0){
			// printf("(%.1f,%.1f)%.1f %.1f\n",xpos,ypos,dist0,dist1);
	 		set_lomap(i,j,0);
	 		set_lomap(i+1,j,1);set_lomap(i-1,j,1);
	 		set_lomap(i,j-1,1);set_lomap(i,j+1,1);

			if ((dist0>1.0)&&(dist1>1.0)){ //HACK TO let the robot go through the door
				...
				set_lomap(i+2,j,2);set_lomap(i-2,j,3);
				set_lomap(i,j-2,2);set_lomap(i,j+2,3);
				set_lomap(i+1,j+1,2);set_lomap(i-1,j+1,3);
				set_lomap(i+1,j-1,2);set_lomap(i-1,j-1,3);
...
void update_lomap_cell(int xo,int yo,int dx, int dy, set<CostNodePair>* Q, int dir0, int dir1){
    ...
    // printf("checking %d,%d: dist %.1f cost %.1f value %.1f\n",xo+dx, xo+dy, dist, cost, nvalue);
		if ((cost<100)&&(nvalue<lomapv[yi*lomap_size+xi])){
