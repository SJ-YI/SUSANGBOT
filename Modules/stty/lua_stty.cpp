...
static int lua_get_crc(lua_State *L) {
	const char *data = lua_tostring(L, 1);
	int len = luaL_checkint(L,2);
	unsigned int sum;
  for ( sum = 0 ; len != 0 ; len-- ) sum += *(data++);
	char buf[8];
	buf[0]=(uint8_t)sum;
...
static int lua_get_int32str(lua_State *L) {
	const int32_t data = luaL_checkint(L,1);
	char buf[4];
	memcpy(buf,&data,4);
...
static int lua_get_int16str(lua_State *L) {
	const int16_t data = luaL_checkint(L,1);
	char buf[2];
	memcpy(buf,&data,2);
...
static const struct luaL_Reg stty_lib [] = {
	...
	{"get_crc", lua_get_crc},
	{"get_int32str", lua_get_int32str},
	{"get_int16str", lua_get_int16str},
	{NULL, NULL}
};
