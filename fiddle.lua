...
if not ok then ok=pcall(dofile, './include.lua') end
if not ok then ok=pcall(dofile, '../../include.lua') end
--SJ: now we use TEAM_ID and PLAYER_ID to assign different names for event channels

-- if PLAYER_ID then print ("Fiddle: player id "..PLAYER_ID) end
...
-- FSM communicationg
----------------------------------------------------------------------------------------------
--SJ: for now body_ch / BodyFSM! for BodyFSM
--Lets attach PLAYER_ID if and only if it exists (to support multiple robots in webots)
--now body_ch = BodyFSM1! , BodyFSM2!, etc
fsm_chs = {}
for sm, en in pairs(Config.fsm.enabled) do
	local fsm_name = sm..'FSM'
	if PLAYER_ID then fsm_name = fsm_name..PLAYER_ID end
